...
if __name__ == '__main__':
    ...
    for threads_num in range(1,9):
        for res in range(fields_MIN, fields_MAX):
            fields = 10
            start = time.time()
            np_mat = finder.payoff_matrix(res, res, fields,threads_num)
            delta_time = time.time() - start
            times[threads_num-1, res-fields_MIN] = delta_time
            times_per_cell[threads_num-1, res-fields_MIN] = (delta_time / (np_mat.shape[0]**2))
            print(threads_num, res, delta_time)
    ...
    save_times(pd_times, "time_threads_res")
    save_times(pd_times_per_cell, "time_threads_res_per_cell")
