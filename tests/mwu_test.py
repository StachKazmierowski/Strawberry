...
from symmetrized.utils import try_reading_matrix_numpy
...
def run_test_symmetric(resources=6, fields=5):
    payoff_matrix = try_reading_matrix_numpy(resources, fields)
    ...
    for i in range(len(phis)):
        for j in range(len(steps_numbers)):
            ...
            error = eval_strategy(payoff_matrix, solution_A, solution_B,
                                         MWU_game_algorithm(resources, resources, fields, phis[i], steps_numbers[j]))
